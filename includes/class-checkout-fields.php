...
final class CheckoutFields {
	...
	public function enqueue_assets(): void {
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					'checking'         => __( 'Vérification en cours…', 'mathisdd-compliant-invoices' ),
					/* translators: %s = legal company name returned by INSEE. */
					'siretValid'       => __( 'SIRET valide : %s', 'mathisdd-compliant-invoices' ),
					'siretInactive'    => __( 'Établissement fermé selon Sirene.', 'mathisdd-compliant-invoices' ),
					'vatValid'         => __( 'TVA valide.', 'mathisdd-compliant-invoices' ),
					/* translators: %s = company name returned by VIES. */
					'vatValidWithName' => __( 'TVA valide : %s', 'mathisdd-compliant-invoices' ),
					'unavailable'      => __( 'Service de validation temporairement indisponible. Réessayez plus tard.', 'mathisdd-compliant-invoices' ),
					'unknownError'     => __( 'Erreur inconnue.', 'mathisdd-compliant-invoices' ),
				),
			)
		);
	}
		...
		<div id="mathisfx-checkout-b2b" class="mathisfx-checkout-b2b">
			<p class="form-row mathisfx-b2b-toggle">
				<label class="checkbox">
					...
					<span><?php esc_html_e( 'Je commande pour mon entreprise', 'mathisdd-compliant-invoices' ); ?></span>
				</label>
			</p>

			<div id="mathisfx_b2b_fields" class="mathisfx-b2b-fields" aria-hidden="true">
				<h3><?php esc_html_e( 'Informations de votre entreprise', 'mathisdd-compliant-invoices' ); ?></h3>
				<p class="mathisfx-b2b-help">
					<?php esc_html_e( 'Ces informations seront utilisées pour générer une facture Factur-X conforme à la réforme française 2026.', 'mathisdd-compliant-invoices' ); ?>
				</p>
				...
				woocommerce_form_field(
					...
					array(
						'type'         => 'text',
						'label'        => __( 'Raison sociale', 'mathisdd-compliant-invoices' ),
						'required'     => false,
				...
				woocommerce_form_field(
					...
					array(
						'type'              => 'text',
						'label'             => __( 'SIRET', 'mathisdd-compliant-invoices' ),
						'placeholder'       => __( '14 chiffres', 'mathisdd-compliant-invoices' ),
						'required'          => false,
				...
				woocommerce_form_field(
					...
					array(
						'type'        => 'text',
						'label'       => __( 'Numéro de TVA intracommunautaire', 'mathisdd-compliant-invoices' ),
				...
				woocommerce_form_field(
					...
					array(
						'type'        => 'text',
						'label'       => __( 'Code APE / NAF (optionnel)', 'mathisdd-compliant-invoices' ),
						'placeholder' => __( 'Ex. 6202A', 'mathisdd-compliant-invoices' ),
						'required'    => false,
						'class'       => array( 'form-row-wide' ),
					),
	...
	public function validate_fields(): void {
		...
		if ( $company_name === '' ) {
			wc_add_notice(
				__( 'Veuillez saisir la raison sociale de votre entreprise.', 'mathisdd-compliant-invoices' ),
				'error'
			);
		}
		...
		if ( $siret === '' ) {
			wc_add_notice(
				__( 'Veuillez saisir le numéro SIRET de votre entreprise.', 'mathisdd-compliant-invoices' ),
				'error'
			);
		} elseif ( ! SiretValidator::is_valid_format( $siret ) ) {
			wc_add_notice(
				__( 'Le numéro SIRET saisi est invalide (14 chiffres requis, échec du contrôle Luhn). Vérifiez la saisie.', 'mathisdd-compliant-invoices' ),
				'error'
			);
		}
		...
		if ( $vat !== '' && ! ViesValidator::is_valid_french_format( $vat ) ) {
			wc_add_notice(
				__( 'Le numéro de TVA intracommunautaire doit suivre le format français : FR suivi de 11 caractères.', 'mathisdd-compliant-invoices' ),
				'error'
			);
		}
	}
