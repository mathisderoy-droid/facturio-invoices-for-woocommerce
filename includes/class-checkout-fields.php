...
final class CheckoutFields {
	...
	public function enqueue_assets(): void {
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					'checking'         => __( 'Vérification en cours…', 'facturio-invoices-for-woocommerce' ),
					/* translators: %s = legal company name returned by INSEE. */
					'siretValid'       => __( 'SIRET valide : %s', 'facturio-invoices-for-woocommerce' ),
					'siretInactive'    => __( 'Établissement fermé selon Sirene.', 'facturio-invoices-for-woocommerce' ),
					'vatValid'         => __( 'TVA valide.', 'facturio-invoices-for-woocommerce' ),
					/* translators: %s = company name returned by VIES. */
					'vatValidWithName' => __( 'TVA valide : %s', 'facturio-invoices-for-woocommerce' ),
					'unavailable'      => __( 'Service de validation temporairement indisponible. Réessayez plus tard.', 'facturio-invoices-for-woocommerce' ),
					'unknownError'     => __( 'Erreur inconnue.', 'facturio-invoices-for-woocommerce' ),
				),
			)
		);
	}
		...
		<div id="mathisfx-checkout-b2b" class="mathisfx-checkout-b2b">
			<p class="form-row mathisfx-b2b-toggle">
				<label class="checkbox">
					...
					<span><?php esc_html_e( 'Je commande pour mon entreprise', 'facturio-invoices-for-woocommerce' ); ?></span>
				</label>
			</p>

			<div id="mathisfx_b2b_fields" class="mathisfx-b2b-fields" aria-hidden="true">
				<h3><?php esc_html_e( 'Informations de votre entreprise', 'facturio-invoices-for-woocommerce' ); ?></h3>
				<p class="mathisfx-b2b-help">
					<?php esc_html_e( 'Ces informations seront utilisées pour générer une facture Factur-X conforme à la réforme française 2026.', 'facturio-invoices-for-woocommerce' ); ?>
				</p>
				...
				woocommerce_form_field(
					...
					array(
						'type'         => 'text',
						'label'        => __( 'Raison sociale', 'facturio-invoices-for-woocommerce' ),
						'required'     => false,
				...
				woocommerce_form_field(
					...
					array(
						'type'              => 'text',
						'label'             => __( 'SIRET', 'facturio-invoices-for-woocommerce' ),
						'placeholder'       => __( '14 chiffres', 'facturio-invoices-for-woocommerce' ),
						'required'          => false,
				...
				woocommerce_form_field(
					...
					array(
						'type'        => 'text',
						'label'       => __( 'Numéro de TVA intracommunautaire', 'facturio-invoices-for-woocommerce' ),
				...
				woocommerce_form_field(
					...
					array(
						'type'        => 'text',
						'label'       => __( 'Code APE / NAF (optionnel)', 'facturio-invoices-for-woocommerce' ),
						'placeholder' => __( 'Ex. 6202A', 'facturio-invoices-for-woocommerce' ),
						'required'    => false,
						'class'       => array( 'form-row-wide' ),
					),
	...
	public function validate_fields(): void {
		...
		if ( $company_name === '' ) {
			wc_add_notice(
				__( 'Veuillez saisir la raison sociale de votre entreprise.', 'facturio-invoices-for-woocommerce' ),
				'error'
			);
		}
		...
		if ( $siret === '' ) {
			wc_add_notice(
				__( 'Veuillez saisir le numéro SIRET de votre entreprise.', 'facturio-invoices-for-woocommerce' ),
				'error'
			);
		} elseif ( ! SiretValidator::is_valid_format( $siret ) ) {
			wc_add_notice(
				__( 'Le numéro SIRET saisi est invalide (14 chiffres requis, échec du contrôle Luhn). Vérifiez la saisie.', 'facturio-invoices-for-woocommerce' ),
				'error'
			);
		}
		...
		if ( $vat !== '' && ! ViesValidator::is_valid_french_format( $vat ) ) {
			wc_add_notice(
				__( 'Le numéro de TVA intracommunautaire doit suivre le format français : FR suivi de 11 caractères.', 'facturio-invoices-for-woocommerce' ),
				'error'
			);
		}
	}
