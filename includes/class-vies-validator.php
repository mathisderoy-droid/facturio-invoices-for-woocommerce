...
final class ViesValidator {
	...
	public static function lookup( string $vat ): array {
		...
		if ( strlen( $vat ) < 4 ) {
			return array(
				'valid' => false,
				'error' => __( 'Numéro de TVA trop court.', 'mathisdd-compliant-invoices' ),
			);
		}
		...
		if ( ! preg_match( '/^[A-Z]{2}$/', $country ) ) {
			return array(
				'valid' => false,
				'error' => __( 'Préfixe pays invalide.', 'mathisdd-compliant-invoices' ),
			);
		}
		...
		if ( 'FR' === $country && ! self::is_valid_french_format( $vat ) ) {
			return array(
				'valid' => false,
				'error' => __( 'Numéro de TVA français mal formé (format attendu : FR + 11 caractères).', 'mathisdd-compliant-invoices' ),
			);
		}
	...
	private static function call_vies( string $country, string $number ): array {
		...
		if ( is_wp_error( $response ) ) {
			return array(
				...
				'error'     => sprintf( __( 'Erreur réseau VIES : %s', 'mathisdd-compliant-invoices' ), $response->get_error_message() ),
				'cacheable' => false,
			);
		}
		...
		if ( $code !== 200 ) {
			return array(
				...
				'error'     => sprintf( __( 'Erreur API VIES (HTTP %d).', 'mathisdd-compliant-invoices' ), $code ),
				'cacheable' => false,
			);
		}
		...
		if ( preg_match( '#<(?:[\w-]+:)?valid\b[^>]*>\s*(true|false)\s*</(?:[\w-]+:)?valid>#i', $raw_body, $match ) ) {
			...
			return array(
				'valid'     => false,
				'error'     => __( 'Réponse VIES illisible.', 'mathisdd-compliant-invoices' ),
				'cacheable' => false,
			);
		}
		...
		if ( ! $is_valid ) {
			$result['error'] = __( 'Numéro de TVA non reconnu par VIES.', 'mathisdd-compliant-invoices' );
		}
		return $result;
	}
	...
	private static function detect_soap_fault( string $body ): ?array {
		...
		if ( in_array( $fault_code, $transient_codes, true ) ) {
			return array(
				'valid'       => false,
				'unavailable' => true,
				'error'       => __( 'Service VIES temporairement saturé (taux d\'appels limité côté UE). Réessayez dans quelques secondes.', 'mathisdd-compliant-invoices' ),
				'cacheable'   => false,
			);
		}
		...
		if ( $fault_code === 'INVALID_INPUT' ) {
			return array(
				'valid'     => false,
				'error'     => __( 'Numéro de TVA mal formé (refusé par VIES).', 'mathisdd-compliant-invoices' ),
				'cacheable' => true,
			);
		}
		...
		return array(
			...
			'error'     => sprintf( __( 'Erreur VIES : %s', 'mathisdd-compliant-invoices' ), $fault_code ),
			'cacheable' => false,
		);
	}
