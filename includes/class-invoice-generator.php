...
final class InvoiceGenerator {
	...
	public function __construct() {
		...
		if ( ! in_array( $trigger, array( 'processing', 'completed' ), true ) ) {
			$trigger = 'completed';
		}
		...
		add_action( 'woocommerce_order_status_' . $trigger, array( $this, 'maybe_generate_invoice' ), 5, 2 );
	}
	...
	private function save_to_uploads( string $binary, string $invoice_number ): array {
		...
		if ( ! empty( $upload['error'] ) ) {
			throw new \RuntimeException( esc_html( 'wp_upload_dir error: ' . $upload['error'] ) );
		}
		...
		if ( ! wp_mkdir_p( $abs_dir ) ) {
			throw new \RuntimeException( esc_html( "Impossible de créer le dossier {$abs_dir}" ) );
		}
		...
		// Direct write of a raw binary PDF; WP_Filesystem offers no benefit
		// here and complicates byte-exact verification.
		$bytes_written = file_put_contents( $abs_path, $binary ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
		if ( $bytes_written === false || $bytes_written !== strlen( $binary ) ) {
			throw new \RuntimeException( esc_html( "Échec écriture du PDF dans {$abs_path}" ) );
		}
	...
	private function create_invoice_post(
		...
		if ( is_wp_error( $post_id ) || ! $post_id ) {
			...
			throw new \RuntimeException( esc_html( "Échec création CPT mathisfx_invoice: {$msg}" ) );
		}
