...
final class AdminDownload {
	...
	public function serve_download(): void {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'Accès refusé.', 'mathisdd-compliant-invoices' ), '', array( 'response' => 403 ) );
		}

		$post = get_post( $invoice_id );
		if ( ! $post || $post->post_type !== InvoicePostType::POST_TYPE ) {
			wp_die( esc_html__( 'Facture introuvable.', 'mathisdd-compliant-invoices' ), '', array( 'response' => 404 ) );
		}
		...
		if ( $rel_path === '' || ! file_exists( $abs_path ) || ! is_readable( $abs_path ) ) {
			wp_die( esc_html__( 'Le fichier PDF est introuvable sur le serveur.', 'mathisdd-compliant-invoices' ), '', array( 'response' => 404 ) );
		}
	...
	public function handle_regenerate(): void {
		...
		if ( ! $order instanceof \WC_Order ) {
			wp_die( esc_html__( 'Commande introuvable.', 'mathisdd-compliant-invoices' ), '', array( 'response' => 404 ) );
		}
	...
	public function maybe_show_regenerate_notice(): void {
		...
		if ( $flag === 'error' ) {
			printf(
				...
				esc_html__( 'La régénération de la facture a échoué. Consultez le journal de débogage pour le détail.', 'mathisdd-compliant-invoices' )
			);
			return;
		}
		...
		printf(
			...
			esc_html__( 'Facture régénérée avec succès. L\'ancien PDF reste archivé.', 'mathisdd-compliant-invoices' )
		);
	}
}
