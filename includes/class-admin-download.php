...
final class AdminDownload {
    ...
    public function __construct() {
        add_action('admin_post_mathisfx_download_invoice',   [$this, 'serve_download']);
        add_action('admin_post_mathisfx_regenerate_invoice', [$this, 'handle_regenerate']);

        // Admin notice shown after a regenerate round-trip.
        add_action('admin_notices', [$this, 'maybe_show_regenerate_notice']);
    }
    ...
    public function serve_download(): void {
        ...
        if (!current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('Accès refusé.', 'factur-x-for-woocommerce'), '', ['response' => 403]);
        }

        $post = get_post($invoice_id);
        if (!$post || $post->post_type !== InvoicePostType::POST_TYPE) {
            wp_die(esc_html__('Facture introuvable.', 'factur-x-for-woocommerce'), '', ['response' => 404]);
        }
        ...
        if ($rel_path === '' || !file_exists($abs_path) || !is_readable($abs_path)) {
            wp_die(esc_html__('Le fichier PDF est introuvable sur le serveur.', 'factur-x-for-woocommerce'), '', ['response' => 404]);
        }
    ...
    public function handle_regenerate(): void {
        ...
        if (!$order instanceof \WC_Order) {
            wp_die(esc_html__('Commande introuvable.', 'factur-x-for-woocommerce'), '', ['response' => 404]);
        }
