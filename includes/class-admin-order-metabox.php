...
final class AdminOrderMetabox {
	...
	public function register(): void {
		...
		add_meta_box(
			...
			__( 'Facture Factur-X', 'mathisdd-compliant-invoices' ),
			array( $this, 'render' ),
	...
	private function render_empty_state( \WC_Order $order ): void {
		echo '<p>' . esc_html__( 'Aucune facture générée pour cette commande.', 'mathisdd-compliant-invoices' ) . '</p>';

		$url = AdminDownload::get_regenerate_url( $order->get_id() );
		printf(
			'<p><a href="%s" class="button button-primary">%s</a></p>',
			esc_url( $url ),
			esc_html__( 'Générer la facture maintenant', 'mathisdd-compliant-invoices' )
		);
		echo '<p class="description">' . esc_html__( 'Consomme le prochain numéro de facture et crée le PDF Factur-X conforme EN 16931.', 'mathisdd-compliant-invoices' ) . '</p>';
	}
	...
	private function render_filled_state( \WC_Order $order, string $invoice_number, int $invoice_post_id ): void {
		...
		if ( $filesize > 0 ) {
			printf(
				'<tr><th style="text-align:left;padding:6px;">%s</th><td style="padding:6px;">%s</td></tr>',
				esc_html__( 'Taille', 'mathisdd-compliant-invoices' ),
		...
		printf(
			...
			esc_html__( 'Télécharger le PDF', 'mathisdd-compliant-invoices' )
		);
		printf(
			'<p><a href="%s" class="button" style="width:100%%;text-align:center;" onclick="return confirm(\'%s\');">%s</a></p>',
			esc_url( $regenerate_url ),
			esc_attr__( 'La facture actuelle est conservée comme archive (obligation légale). Un nouveau numéro sera attribué à la nouvelle version. Continuer ?', 'mathisdd-compliant-invoices' ),
			esc_html__( 'Régénérer la facture', 'mathisdd-compliant-invoices' )
		);
	}
}
